...
export default function CmsPostEngagementManager({
    ...
    const reload = useCallback(async () => {
        if (section === 'comments') {
            const response = await apiGetCmsPostComments(entryId)
            setComments(response.data ?? [])
            return
        }
        if (section === 'ratings') {
            const response = await apiGetCmsPostRatings(entryId)
            setRatings(response.data ?? [])
            setSummary(response.summary)
            return
        }
        const response = await apiGetCmsPostFaqs(entryId, locale)
        setFaqs(response.data ?? [])
    }, [entryId, locale, section])
        ...
        <div className="space-y-6">
            <AdaptiveCard>
                <div className="mb-4 flex items-center justify-between">
                    <div>
                        <h5 className="capitalize">{section}</h5>
                        <p className="text-sm text-gray-500">
                            Manage manual and visitor-submitted {section}.
                        </p>
                    </div>
                    <Select
                        className="w-36"
