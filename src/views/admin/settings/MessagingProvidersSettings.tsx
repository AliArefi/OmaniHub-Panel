...
export default function MessagingProvidersSettings() {
    ...
    const { data, mutate } = useSWR('admin-settings-messaging-providers', () => apiGetSingletonSettings<{ data: Provider[]; provider_definitions: Record<string, { fields?: Field[] }> }>('messaging-providers'))
    ...
    const save = async () => {
        ...
        try {
            await apiUpdateSingletonSettings('messaging-providers', { providers: providers.map((provider) => {
                ...
                const fields = data?.provider_definitions?.[provider.provider_type]?.fields ?? []
